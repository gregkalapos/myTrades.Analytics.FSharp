...
module UltimateOscillator =
    ...
    let private FsAsyncUltimateOscillator (prices: seq<OHCLWithDate>) (cT: Threading.CancellationToken) =
        async
            {
             ...
             return ultimateOscillatorHelper prices 0 [] [] [] 0 [] [] []
            }

    let UltimateOscillatorAsync (prices: seq<OHCLWithDate>) (cT: Threading.CancellationToken) =
        Async.StartAsTask(FsAsyncUltimateOscillator prices cT)

    let UltimateOscillator (prices: seq<OHCLWithDate>) =
        (FsAsyncUltimateOscillator prices (new Threading.CancellationToken())) |> Async.RunSynchronously